(ns com.github.ivarref.demo-server
  (:require [aleph.http]
            [cheshire.core]
            [com.github.ivarref.locksmith :as locksmith]
            [com.github.ivarref.yasp]))

(defonce the-keys (locksmith/gen-certs {:duration-days 7}))
(def server-keys (str (get the-keys :ca-cert)
                      (get the-keys :server-cert)
                      (get the-keys :server-key)))

(def client-keys (str (get the-keys :ca-cert)
