...
#_(defn tls-pump [proxy-cfg {:keys [^Socket sock]}]
    (try
...
(t/deftest tls-hello
  (let [st (atom {})
        [server-keys client-keys] (locksmith/certs-array-2 {})
        proxy-cfg {:state          st
    ...
    (try
      (with-open [remote-server (s/start-server! (atom {}) {:local-port 9999} s/echo-handler)]
        ...
        (yasp/tls-proxy! proxy-cfg {:op      "connect"
                         ...
                         #_(let [#_tls-context (tls/ssl-context-or-throw client-keys nil)
                                 _]
                             #_(with-open [sock (tls/socket tls-context "localhost" @tls-decode-server 3000)]
...
#_(t/deftest bad-tls-str-server-1
    (let [st (atom {})
          proxy-cfg {:state          st
                     :allow-connect? (constantly true)
                     :now-ms         1
...
#_(t/deftest bad-tls-str-server-2
    (let [st (atom {})
          proxy-cfg {:state          st
                     :allow-connect? (constantly true)
                     :now-ms         0
...
#_(defn keep-sending [{:keys [^Socket sock closed?]}]
    (try
...
#_(t/deftest pump-threads-exits
    (let [st (atom {})
          kp1 (gen-key-pair)
          proxy-cfg {:state          st
                     :allow-connect? (constantly true)
